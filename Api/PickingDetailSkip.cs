...
namespace Api
{
    public class PickingDetailSkip : ControllerBase
    {

        private readonly string defaultConnection = Environment.GetEnvironmentVariable("DefaultDBConnection");

        [FunctionName("PickingDetailSkip")]
        public async Task<IActionResult> Run(
                [HttpTrigger(AuthorizationLevel.Anonymous, "put", Route = "pickingDetail/Skip/{ShipID}")] HttpRequest req,
                ILogger log, string ShipID)
        {
            try
            {
                ...
                values.Add("@Cmd", "PickingListSkip", dbType: DbType.String, direction: ParameterDirection.Input);
